...
if (process.argv.length >= 3 && (process.argv.at(2) === "--help" || process.argv.at(2) === "-h")) {
    const usage = `
    Usage: ${jsEngine} ${scriptPath} <output file>
    `
...
const text = await randomPassage(minimumLength);

if (process.argv.length >= 3) {
    const outputFile = process.argv.at(2) as string;

    Bun.write(outputFile, text);
}
